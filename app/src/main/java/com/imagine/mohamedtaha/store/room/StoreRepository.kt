...
class StoreRepository @Inject constructor(private val storeDao: StoreDao) {
    ...
    @Suppress("RedundantSuspendModifier")
    @WorkerThread
    suspend fun updateStockWarehouse(id: Long, store_id: Long, first_balance: String, notes: String) {
        storeDao.updateStockWareHouse(id, store_id, first_balance, notes)
    }

    @Suppress("RedundantSuspendModifier")
    @WorkerThread
    suspend fun deleteStockWarehouse(id: Long) {
        storeDao.deleteStockWareHouse(id)
    }

    val getAllStokeWareHouse: Flow<List<StockingHouse>> = storeDao.getAllStokeWareHouse()
    val getAllStokeWareHouseWitCategoriesAndStoresShow: Flow<List<ShowStockWare>> = storeDao.getAllStokeWareHouseWitCategoriesAndStores()
