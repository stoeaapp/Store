...
class StoreViewModel(private val repository: StoreRepository) : ViewModel() {
    ...
    val allNamePermissionsLiveData: LiveData<List<String>> = map(allPermissionsLiveData) {it.map {
        it.permissionName } }
    ...
    val allNameStoresLiveData : LiveData<List<String>> = map(allStoresLiveData){it.map { it -> it.typeStore }}
    ...
    val AllStokeWareHouseWitCategoriesAndStores: LiveData<List<StockWareWithCategoriesAndStores>> = repository.getAllStokeWareHouseWitCategoriesAndStores.asLiveData()
